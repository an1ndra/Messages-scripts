...
while IFS=$'\t' read -r y label; do
    if [ "$y" -gt 350 ] 2>/dev/null; then ROW_Y="$y"; ROW_LABEL="$label"; break; fi
done < <(grep -oE 'text="[^"]{2,}"[^>]*bounds="\[[0-9]+,[0-9]+\]\[[0-9]+,[0-9]+\]"' "$TMP/ui.xml" \
    | sed -E 's/text="([^"]*)".*\[[0-9]+,([0-9]+)\]\[[0-9]+,([0-9]+)\]/\t\1\t\2\t\3/' \
    | awk -F'\t' '{print int(($3+$4)/2)"\t"$2}' | sort -n)
[ -z "$ROW_Y" ] && fail "no conversation row found on home"
echo "[target] conversation row '$ROW_LABEL' at y=$ROW_Y"
adb_ shell input tap 540 "$ROW_Y"
sleep 3

# status line renders only under the LAST own message (visible at list bottom)
dump_ui || fail "chat dump failed"
grep -qE 'text="[0-9]{1,2}:[0-9]{2} [AP]M • SMS"' "$TMP/ui.xml" || fail "send-status line missing on last own message"
